...
ranges = {
    "Land": range(min(deck["Land"], 7) + 1),
    "Grief": range(min(deck["Grief"], 7) + 1),
    "Fury": range(min(deck["Fury"], 7) + 1),
    "Undying": range(min(deck["Undying"], 7) + 1),
    "BR": range(min(deck["BR"], 7) + 1),
    "Black": range(min(deck["Black"], 7) + 1),
    "Red": range(min(deck["Red"], 7) + 1),
}

for Land, Grief, Fury, Undying, BR, Black, Red in product(*ranges.values()):
    ...
    if cards_drawn == 7:
        ...
        scammed_Grief = (
            Land >= 1 and Grief >= 1 and Undying >= 1 and black_pitch_spells >= 1
        )
        scammed_Fury = (
            Land >= 1 and Fury >= 1 and Undying >= 1 and red_pitch_spells >= 1
        )
        ...
        if Land >= 1 and Grief >= 1 and Undying >= 1 and black_pitch_spells == 0:
            prob_to_topdeck_Pitch = (
                deck["Grief"] - Grief + deck["Undying"] - Undying + BR + Black
            ) / 53
        ...
        if scammed_Grief or scammed_Fury:
            ...
            # Is Grief an out?
            if (
                Land >= 1 and Grief == 0 and Undying >= 1 and black_pitch_spells >= 1
            ) or (
                Land >= 1 and Grief >= 1 and Undying >= 1 and black_pitch_spells == 0
            ):
                prob_to_topdeck_Grief = deck["Grief"] / 53
                scammed_Grief_or_Fury_prob_draw += hand_prob * prob_to_topdeck_Grief
            # Is Fury an out?
            if (Land >= 1 and Fury == 0 and Undying >= 1 and red_pitch_spells >= 1) or (
                Land >= 1 and Fury >= 1 and Undying >= 1 and red_pitch_spells == 0
            ):
                prob_to_topdeck_Fury = deck["Fury"] / 53
                scammed_Grief_or_Fury_prob_draw += hand_prob * prob_to_topdeck_Fury
            # Is Undying an out?
            need_Undying_to_cast = (
                Land >= 1
                and Undying == 0
                and (
                    (Grief >= 1 and black_pitch_spells >= 1)
                    or (Fury >= 1 and red_pitch_spells >= 1)
                )
            )
            need_Undying_to_pitch = (
                Land >= 1 and Undying == 1 and Grief == 1 and black_pitch_spells == 0
            )
            if need_Undying_to_cast or need_Undying_to_pitch:
                prob_to_topdeck_Undying = deck["Undying"] / 53
                scammed_Grief_or_Fury_prob_draw += hand_prob * prob_to_topdeck_Undying
            # Is BR an out?
            Elemental_but_no_pitch_possible = (
                Grief == 1 and black_pitch_spells == 0
            ) or (Fury == 1 and red_pitch_spells == 0)
...
print(
    f"Probability of turn one scammed Grief on the play: {scammed_Grief_prob_play * 100:.1f}%."
)
no_scammed_Grief_prob_play = 1 - scammed_Grief_prob_play
print(
    f"With aggressive mulligans to 6, this probability is: {(1 - no_scammed_Grief_prob_play *  no_scammed_Grief_prob_play) * 100:.1f}%."
)
print(
    f"With aggressive mulligans to 5, this probability is: {(1 - no_scammed_Grief_prob_play *  no_scammed_Grief_prob_play * no_scammed_Grief_prob_play ) * 100:.1f}%."
)
print(
    f"With aggressive mulligans to 4, this probability is: {(1 - no_scammed_Grief_prob_play *  no_scammed_Grief_prob_play * no_scammed_Grief_prob_play * no_scammed_Grief_prob_play ) * 100:.1f}%."
)
print(
    f"Probability of turn one scammed Grief on the draw: {scammed_Grief_prob_draw * 100:.1f}%."
)
print("-" * 30)

print(
    f"Probability of turn one scammed Grief or Fury on the play: {scammed_Grief_or_Fury_prob_play * 100:.1f}%."
)
no_scammed_Grief_or_Fury_prob_play = 1 - scammed_Grief_or_Fury_prob_play
print(
    f"With aggressive mulligans to 6, this probability is: {(1 - no_scammed_Grief_or_Fury_prob_play *  no_scammed_Grief_or_Fury_prob_play) * 100:.1f}%."
)
print(
    f"With aggressive mulligans to 5, this probability is: {(1 - no_scammed_Grief_or_Fury_prob_play *  no_scammed_Grief_or_Fury_prob_play * no_scammed_Grief_or_Fury_prob_play ) * 100:.1f}%."
)
print(
    f"With aggressive mulligans to 4, this probability is: {(1 - no_scammed_Grief_or_Fury_prob_play *  no_scammed_Grief_or_Fury_prob_play * no_scammed_Grief_or_Fury_prob_play * no_scammed_Grief_or_Fury_prob_play ) * 100:.1f}%."
)
print(
    f"Probability of turn one scammed Grief or Fury on the draw: {scammed_Grief_or_Fury_prob_draw * 100:.1f}%."
)
print("-" * 30)
...
print("=====")
print(
    "For each deck, we give two probabilities. First is holding a pitcher or another Force, conditional on holding Force. Second is having a choice between what blue card to pitch."
)
...
for Pitcher_deck in range(23):
    ...
    deck = {"Force": 4, "Pitcher": Pitcher_deck, "Other": 60 - 4 - Pitcher_deck}
    ...
    print(
        f"Deck with 4 Forces and {Pitcher_deck} pitchers; probability : {(Combo_Success_prob/Force_prob)* 100:.1f}, {(Choice_prob/Force_prob)* 100:.1f}%."
    )
